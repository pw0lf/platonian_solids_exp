...
class MultiheadPCA(nn.Module):
    ...
    def forward(self, x_source, x_target, mask):
        ...
        for i in range(self.num_heads):
            ...
            outputs[:, t:(t + self.target_dims)] = self.att_layers[i](
                x_source[:, s:(s + self.source_dims)], x_target[:, t:(t + self.target_dims)], mask)
        return outputs
...
class PairwiseAttentionTransformer(nn.Module):
    ...
    def forward(self, sources, x_target, masks):
        x_t1 = self.target_layernorm(x_target)
        x_s1_list = [ln(x_s) for ln,x_s in zip(self.layer_norms1, sources)]
        x_2_list = [att(x_s,x_t1,n) for att, x_s, n in zip(self.att_layers, x_s1_list, masks)]
...
class CellularTransformer(nn.Module):
    ...
    def forward(self, x_0, x_1, x_2, adj00, icd01, adj11, icd02, icd12, adj22, node_counts):
        ...
        # Densify each incidence/adjacency matrix to a bool mask exactly once per
        # forward pass. Previously PCAttention.forward converted its `neighborhood`
        # sparse tensor with `.to_dense().bool()` on every call, i.e.
        # num_layers * num_heads (= 64) times for the *same* matrix. Hoisting the
        # conversion here cuts that to a single pass per matrix; the three transposed
        # neighborhoods reuse the dense tensor as a `.T` view (no extra densification).
        adj00_m = adj00.to_dense().bool()
        icd01_m = icd01.to_dense().bool()
        adj11_m = adj11.to_dense().bool()
        icd02_m = icd02.to_dense().bool()
        icd12_m = icd12.to_dense().bool()
        adj22_m = adj22.to_dense().bool()

        #pat layers
        for i in range(self.num_layers):
            x_0_new = self.pat_layers_rk0[i]([x_0,x_1,x_2],x_0,[adj00_m, icd01_m, icd02_m])
            x_1_new = self.pat_layers_rk1[i]([x_0,x_1,x_2],x_1,[icd01_m.T, adj11_m, icd12_m])
            x_2_new = self.pat_layers_rk2[i]([x_0,x_1,x_2],x_2,[icd02_m.T, icd12_m.T, adj22_m])
