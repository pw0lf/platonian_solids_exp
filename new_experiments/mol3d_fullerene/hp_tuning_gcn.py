"""Small hyperparameter search for GCN on mol3d_fullerene (combined mol3d +
fullerene dataset). See mol3d/hp_tuning_gcn.py for the shared rationale.

Carves a small hp-train/hp-val pair out of BOTH data sources' TRAIN portions
(mol3d_split["train"] and fullerene_split["train_idx"]) and combines them,
mirroring how the real exp_gnn.py trains on the concatenation of both. Never
loads either source's test split.
"""
import argparse
import json
import sys
...
if __name__ == "__main__":
    ...
    # Carve hp-train/hp-val from each source's TRAIN portion only; test
    # indices from either source are never referenced by this script.
