...
if __name__ == "__main__":
    ...
    parser.add_argument("--hp_file",         type=str,   default=None,
                        help="JSON from fullerene/hp_tuning_fullerenet.py (shared tuning). Values "
                             "for keys present in the file unconditionally override this script's "
                             "CLI defaults for lr/hidden_channels/num_conv_layer/dropout/"
                             "node_embedding/num_heads -- even if you also pass those flags "
                             "explicitly.")
    ...
    if args.hp_file:
        with open(args.hp_file) as f:
            hp = json.load(f)
        hp_keys = ("lr", "hidden_channels", "num_conv_layer", "dropout", "node_embedding", "num_heads")
        for key in hp_keys:
            if key in hp:
