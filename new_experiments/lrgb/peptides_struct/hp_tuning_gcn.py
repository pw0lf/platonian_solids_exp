"""Small hyperparameter search for GCN on lrgb/peptides_struct.

Unlike mol3d/fullerene, LRGB already ships separate train/val/test splits
(no split.json to carve from) -- this script subsamples the REAL train and
REAL val splits directly (see hp_search_utils.subsample), rather than
carving a new val out of train. Test is never loaded at all.
"""
import argparse
import json
import sys
...
def make_model(cfg):
    return GCN(
        ...
        num_readout_layers=cfg["num_readout_layers"], out_dim=OUT_DIM, dropout=cfg["dropout"],
    )
