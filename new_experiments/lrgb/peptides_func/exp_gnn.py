...
def evaluate(model, loader, device, use_edge_attr):
    model.eval()
...
if __name__ == "__main__":
    ...
    if args.output is None:
        args.output = f"results_func_{args.model.lower()}.json"
    ...
    for run in range(3):
        ...
        val_ap,  _,          _            = evaluate(model, val_loader,  device, use_edge_attr)
        test_ap, test_preds, test_targets = evaluate(model, test_loader, device, use_edge_attr)
