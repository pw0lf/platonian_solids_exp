"""Small hyperparameter search for CIN++ (variant="CINpp") on mol3d.

Identical to hp_tuning_cin.py except variant="CINpp" (adds down-messages).
...
results/best_hp_cinpp.json for exp_cin.py --hp_file to pick up.
"""
import argparse
import json
import sys
...
def make_model(cfg, dims):
    return CIN(
        ...
        num_layers=cfg["num_layers"], hidden=cfg["hidden"], variant="CINpp",
