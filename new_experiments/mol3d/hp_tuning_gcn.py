"""Small hyperparameter search for GCN on mol3d.

Random search over {lr, hidden_channels, num_conv_layers, dropout}, trained
and validated on a small subset carved out of the TRAIN split only (see
hp_search_utils.carve_val_from_train) -- this script never loads or
references the test split at all. Saves the winning config to
results/best_hp_gcn.json for exp_gnn.py --hp_file to pick up.
"""
import argparse
import json
import sys
...
from mol3d_gnn import load_gnn_data, NODE_DIM
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output",     type=str, default="best_hp_gcn.json")
    ...
    if torch.cuda.is_available():
        ...
        device = "cpu"
    print(f"Device: {device} | tuning: GCN")
    ...
    train_data, _ = load_gnn_data(hp_train_idx, root=args.datapath, load_edge_features=False)
    val_data, _   = load_gnn_data(hp_val_idx,   root=args.datapath, load_edge_features=False)
