...
################# HP-Tuning #################
def hp_optimization(model, hps, data_params, n_trials, random_seed, device="cpu"):
	fixed_params = {}
	for hp in hps:
		if hp[1] == "fixed":
	...
	def stop_if_perfect(study, trial):
		if trial.value is not None and trial.value >= 100.0:
			study.stop()

	study = optuna.create_study(direction="maximize")
	study.optimize(objective, n_trials=n_trials, callbacks=[stop_if_perfect])
...
if __name__ == "__main__":
	...
	n_trials = 10
	study_dataframe, best_hps = hp_optimization(model, hps, data_params, n_trials, SEED, device=DEVICE)
